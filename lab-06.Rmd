...
diamonds_premium <- diamonds_clean %>%
  filter(cut == "Premium")

view(diamonds_premium)
...
diamonds %>%
  group_by(clarity) %>%
  summarise(mean_clarity = mean(table_mm))
...
The relationship between clarity and average table length is that the lower the clarity score the bigger the diamond table length is on average. 
